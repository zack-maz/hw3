...
Node* llfilter(Node* head, Comp pred)
{
    ...
    if (head == nullptr) return nullptr; //end recursion calls
    Node* temp = llfilter(head->next, pred); //recurse down to end
    if (pred(head->val))
    {
        delete head;
        return temp; //return current node's next
    }

    head->next = temp; //temp will be the next value if it wasnt deleted
    return head; //because head is returned instead of temp
